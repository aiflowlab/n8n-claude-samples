...
**事業者・士業事務所・小規模チームの定例業務を、AI ワークフローで自動化するサンプル集です。**

- **Sample01 — 問い合わせメール 自動仕分け Bot**: 受信メールを自動分類 → 緊急度別の返信案を生成 → Slack 通知(月 1,000 件で約 500 円、テスト 10/10 全件一致)
- **Sample02 — クレカ経費 自動記録 Bot**: クレカ明細メールを自動検知 → AI が経費かどうか判定 → 帳簿に記録 + Slack 通知(月 200 件で約 200 円、テスト 7/7 全件 PASS)

各サンプルにはワークフロー定義 / プロンプト / テストコード / スクリーンショットまで一式収録。設定ファイルを書き換えるだけで、業種に合わせてカスタマイズできます。「自分の業務に合わせて構築してほしい」というご相談は本ページ末尾の[連絡先](#連絡先)まで。

## サンプル一覧

| サンプル | 概要 | 状態 |
|---|---|---|
| [Sample01: Inquiry Auto-Router(問い合わせメール 自動仕分け Bot)](./samples/sample01/) | 問い合わせメール自動分類 + 緊急度別 返信ドラフト + Slack 通知 | ✓ 完成(2026-05-08) |
| [Sample02: Expense Auto-Detector(クレカ経費 自動記録 Bot)](./samples/sample02/) | クレカ明細メール自動検知 + AI 経費判定 + 帳簿追記 + Slack 通知 | ✓ 完成(2026-05-13) |
...
- 顧客窓口・受付業務で定型対応に時間を取られている事業者・士業事務所
- 小規模チームで「メールを誰が見るか」「どう優先度を付けるか」が属人化している
- LLM × ワークフロー自動化の実装パターンを評価したい技術担当者
...
> **セットアップ手順 — 動かす予定がなければ、ここから先は読み飛ばして OK です。**
> 「自分の業務に合わせて構築してほしい」というご相談は[連絡先](#連絡先)までお気軽にどうぞ。
...
### 本サンプルの位置づけ(Human-in-the-loop 設計)

AI による分類・按分率はあくまで「提案」であり、確定処理ではありません。**勘定科目・税区分・按分率の最終確定は、利用者または顧問税理士でご確認ください**。本サンプルは「AI が下書き → Slack 通知 → 人間がレビュー → 必要に応じて帳簿を修正」を前提に設計しています。AI 判定行は `備考` 欄に `⚠ 要レビュー` マーカーが残るため、月末・申告前の見直し起点として使えます。

### こんな方におすすめ

- 中小事業者・士業事務所・小規模チームで経費の手入力・記録漏れに悩んでいる
- 経理担当者の月末作業を「データ入力」から「AI 結果のレビュー」に置き換えたい
- ルール + AI ハイブリッドの判定設計パターンを評価したい技術担当者
...
**Sample02**
- note: [経費の記録漏れをゼロにした話。クレカ明細メールが届いたら AI が自動で帳簿に転記してくれる【コード公開】](https://note.com/aiflowlab/n/n106343f2dd1a)
- Zenn: [Claude Haiku 4.5 + n8n でクレカ明細メールを自動経費記録したら、7/7 精度・1 件約 1 円で運用できた](https://zenn.dev/aiflowlab/articles/n8n-claude-credit-expense-automation)
